...
object ConsoleLogger {
  ...
  def makeWithName(name: String)(logFormat: (LogContext, => String) => String)
    : ZLayer[Clock, Nothing, Logging] =
    make((context, line) =>
      logFormat(context.annotate(LogAnnotation.Name, name :: Nil), line))

  def make(logFormat: (LogContext, => String) => String)
    : ZLayer[Clock, Nothing, Logging] =
    Logging.make { (context, line) =>
      for {
        ...
        msg = (date.toString + " " + level.render + " " + loggerName + " " + logFormat(
          context,
          line))
        _ <- level match {
